//
// Created by Giovane Avancini on 06/02/24.
//
...
TPZMixedCompressibleDarcyFlow::TPZMixedCompressibleDarcyFlow() : TPZRegisterClassId(&TPZMixedCompressibleDarcyFlow::ClassId),
                                         TBase(), fDim(-1), fAlpha(0.) {}

[[maybe_unused]] TPZMixedCompressibleDarcyFlow::TPZMixedCompressibleDarcyFlow(int id, int dim, REAL alpha) : TPZRegisterClassId(&TPZMixedCompressibleDarcyFlow::ClassId),
                                                        TBase(id), fDim(dim), fAlpha(alpha)
{
}
...
/**
         copy constructor
 */
TPZMixedCompressibleDarcyFlow::TPZMixedCompressibleDarcyFlow(const TPZMixedCompressibleDarcyFlow &copy) : TBase(copy), fDim(copy.fDim), fAlpha(copy.fAlpha)
{   
}
/**
         copy constructor
 */
TPZMixedCompressibleDarcyFlow &TPZMixedCompressibleDarcyFlow::operator=(const TPZMixedCompressibleDarcyFlow &copy)
{
    TBase::operator=(copy);
    fDim = copy.fDim;
    fAlpha = copy.fAlpha;
...
void TPZMixedCompressibleDarcyFlow::Contribute(const TPZVec<TPZMaterialDataT<STATE>> &datavec, REAL weight, TPZFMatrix<STATE> &ek,
                                   TPZFMatrix<STATE> &ef) {
    ...
    for (int ip = 0; ip < phrp; ip++) {
        ...
        ek(phrq+ip, phrq+ip) += -fAlpha*weight;
    }
...
void TPZMixedCompressibleDarcyFlow::ContributeBC(const TPZVec<TPZMaterialDataT<STATE>> &datavec, REAL weight, TPZFMatrix<STATE> &ek,
...
void TPZMixedCompressibleDarcyFlow::Solution(const TPZVec<TPZMaterialDataT<STATE>> &datavec, int var, TPZVec<STATE> &solOut) {
...
void TPZMixedCompressibleDarcyFlow::Errors(const TPZVec<TPZMaterialDataT<STATE>> &data, TPZVec<REAL> &errors) {
...
int TPZMixedCompressibleDarcyFlow::VariableIndex(const std::string &name) const {
...
int TPZMixedCompressibleDarcyFlow::NSolutionVariables(int var) const {
    if (var == 1) return 3;
    if (var == 2) return 1;
    if (var == 3) return 3;
    if (var == 4) return 3;
...
void TPZMixedCompressibleDarcyFlow::SetDimension(int dim) {
    if (dim > 3 || dim < 1) DebugStop();
    fDim = dim;
}

void TPZMixedCompressibleDarcyFlow::SetAlpha(REAL alpha) {
    fAlpha = alpha;
}

int TPZMixedCompressibleDarcyFlow::ClassId() const {
    return Hash("TPZMixedCompressibleDarcyFlow") ^ TBase::ClassId() << 1;
}

TPZMaterial *TPZMixedCompressibleDarcyFlow::NewMaterial() const {
    return new TPZMixedCompressibleDarcyFlow(*this);
}

void TPZMixedCompressibleDarcyFlow::Print(std::ostream &out) const {
    out << "Material Name: " << this->Name() << "\n";
    out << "Material Id: " << this->Id() << "\n";
    out << "Dimension: " << this->Dimension() << "\n\n";
}

void TPZMixedCompressibleDarcyFlow::FillDataRequirements(TPZVec<TPZMaterialDataT<STATE>> &datavec) const {
...
void
TPZMixedCompressibleDarcyFlow::FillBoundaryConditionDataRequirements(int type, TPZVec<TPZMaterialDataT<STATE>> &datavec) const {
